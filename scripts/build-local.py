...
def detect_default_workspace(root: Path) -> Path | None:
    if root.as_posix().startswith("/mnt/"):
        return Path("/tmp/l4d2-player-skills-build")
    return None
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compile L4D2-Player-Skills SourceMod plugins into a local build directory.")
